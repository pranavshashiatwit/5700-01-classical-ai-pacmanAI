...
class TextGroup(object):
    ...
    def setupText(self):
        size = TILEHEIGHT
        self.alltext[READYTXT] = Text("READY!", YELLOW, 11.25 * TILEWIDTH, 17 * TILEHEIGHT, size, visible=False)
        self.alltext[PAUSETXT] = Text("PAUSED!", YELLOW, 10.625 * TILEWIDTH, 17 * TILEHEIGHT, size, visible=False)
        self.alltext[GAMEOVERTXT] = Text("GAMEOVER!", YELLOW, 10 * TILEWIDTH, 20 * TILEHEIGHT, size, visible=False)
        self.alltext[METRICSTXT] = Text("", WHITE, 10, SCREENSIZE[1] - TILEHEIGHT, size)
